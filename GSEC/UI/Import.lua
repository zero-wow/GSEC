...
importframe:SetTitle("GSEC: Import a Macro String.")
importframe:SetStatusText(L["Import Macro from Forums"])
importframe:SetCallback("OnClose", function(widget)
  importframe:Hide()
  if importframe.ReturnToControl then
    importframe.ReturnToControl = false
    if GSE.ControlPanel and GSE.ControlPanel.frame then GSE.ControlPanel.frame:Show() end
  else
    GSE.GUIShowViewer()
  end
end)
...
local function queueImportComplete()
  local event = {}
  if importframe.ReturnToControl then
    event.action = "refreshcontrol"
    importframe.ReturnToControl = false
  else
