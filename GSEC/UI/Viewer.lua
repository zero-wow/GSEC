...
local function resetViewerSelection()
  editkey = ""
  viewframe.SequenceName = ""
  viewframe.ClassID = 0
  if viewframe.EditButton then
    viewframe.EditButton:SetDisabled(true)
  end
  if viewframe.ExportButton then
...
function viewframe:clearpanels(widget, selected)
  ...
  for k,v in pairs(viewframe.panels) do
    ...
    if k == widget:GetKey() then
      ...
      if viewframe.panels[k] and viewframe.panels[k].SetClicked then
        viewframe.panels[k]:SetClicked(selected and true or false)
      end
    else
      GSE.PrintDebugMessage ("other widget key", "GUI")
      GSE.PrintDebugMessage("reprinting k " .. k, "GUI")
      local wid = viewframe.panels[k]
      if wid and wid.SetClicked then
...
function GSE.GUIViewerToolbar(container)
  ...
  updbutton:SetCallback("OnClick", function() 
    local selectedKey = editkey
    if GSE.isEmpty(selectedKey) and not GSE.isEmpty(viewframe.ClassID) and not GSE.isEmpty(viewframe.SequenceName) then
      selectedKey = tostring(viewframe.ClassID) .. "," .. viewframe.SequenceName
    end
    if GSE.isEmpty(selectedKey) then
      viewframe:SetStatusText("Select a sequence to edit.")
      return
    end
    GSE.GUIEditFrame:SetStatusText("")
	GSE.GUILoadEditor(selectedKey, viewframe) 
  end)
...
function GSE.GUIViewerLayout(mcontainer)
  mcontainer:SetStatusText("GSEC: Sequence Viewer")
