...
local function trimStoredSequenceName(sequenceName)
...
local function findStoredSequenceClassID(sequenceName)
...
function GSE.DeleteSequence(classid, sequenceName)
  local resolvedSequenceName = trimStoredSequenceName(sequenceName)
  if GSE.isEmpty(resolvedSequenceName) then
    return false
  end

  local resolvedClassID = GSE.ResolveSequenceClassID(classid)
  if GSELibrary[resolvedClassID] == nil or GSE.isEmpty(GSELibrary[resolvedClassID][resolvedSequenceName]) then
    resolvedClassID = findStoredSequenceClassID(resolvedSequenceName) or resolvedClassID
  end

  GSE.DeleteMacroStub(resolvedSequenceName, true)
  if GSELibrary[resolvedClassID] then
    GSELibrary[resolvedClassID][resolvedSequenceName] = nil
  end
  return true
end
...
function GSE.PerformMergeAction(action, classid, sequenceName, newSequence)
  action = string.upper(action or "")
  classid = GSE.ResolveSequenceClassID(classid, newSequence)

  if action == "IGNORE" then
    return
  end

  if action == "MERGE" or action == "RENAME" then
    GSE.AddSequenceToCollection(sequenceName, newSequence, classid)
    return
  end
...
function GSE.OOCAddSequenceToCollection(sequenceName, sequence, classid)
  ...
  if GSE.isEmpty(sequence.SpecID) then
    ...
    sequence.SpecID = sidy or classid
  end

  if GSE.isEmpty(classid) then
    classid = tonumber(GSE.GetClassIDforSpec(sequence.SpecID)) or 0
  end
  ...
  if (classid == GSE.GetCurrentClassID() or classid == 0) and sequence.MacroVersions and sequence.Default and sequence.MacroVersions[sequence.Default] then
     GSE.UpdateSequence(sequenceName, sequence.MacroVersions[sequence.Default])
  end
  if GSE.ShouldAutoCreateMacroStub(classid) then
...
function GSE.CleanEmptyLibraryEntries()
  if type(GSELibrary) ~= "table" then
    return
  end

  if GSELibrary[""] ~= nil then
    GSELibrary[""] = nil
  end

  for classid, sequences in pairs(GSELibrary) do
    if type(sequences) == "table" and sequences[""] ~= nil then
      sequences[""] = nil
    end
  end
end
...
local function macroBodyMatchesStub(macroname, macroBody)
  if GSE.isEmpty(macroname) or GSE.isEmpty(macroBody) then
    return false
  end
  local trimmedBody = tostring(macroBody):gsub("[^%w ]", "")
  local expectedBody = GSE.CreateMacroString(macroname):gsub("[^%w ]", "")
  return string.lower(trimmedBody) == string.lower(expectedBody)
end
...
function GSE.UpdateMacroString()
  ...
  for macid = 1, maxmacros do
    ...
    if not GSE.isEmpty(mname) then
      if GSELibrary[GSE.GetCurrentClassID()] and not GSE.isEmpty(GSELibrary[GSE.GetCurrentClassID()][mname]) then
        if not macroBodyMatchesStub(mname, mbody) then
          GSE.PrintDebugMessage(string.format("Skipping protected EditMacro update for %s", mname), GNOME)
        end
      end
      if not GSE.isEmpty(GSELibrary[0]) then
...
function GSE.ShouldAutoCreateMacroStub(classid)
  local resolvedClassID = tonumber(classid) or classid
  if resolvedClassID == 0 then
    return GSEOptions.autoCreateMacroStubsGlobal and true or false
  end
  return resolvedClassID == GSE.GetCurrentClassID() and GSEOptions.autoCreateMacroStubsClass and true or false
end
...
function GSE.OOCCheckMacroCreated(SequenceName, create)
  ...
  if macroIndex and macroIndex ~= 0 then
    found = true
    if create then
      local mname, _, mbody = GetMacroInfo(macroIndex)
      if not macroBodyMatchesStub(mname or SequenceName, mbody) then
        GSE.PrintDebugMessage(string.format("Macro %s already exists. Skipping protected EditMacro rewrite.", SequenceName), GNOME)
      end
    end
  else
    if create then
...
function GSE.DeleteMacroStub(sequenceName, force)
  local resolvedSequenceName = trimStoredSequenceName(sequenceName)
  if GSE.isEmpty(resolvedSequenceName) then
    return false
  end

  local macroIndex = GetMacroIndexByName(resolvedSequenceName)
  if not macroIndex or macroIndex <= 0 then
    return false
  end

  local mname, _, mbody = GetMacroInfo(macroIndex)
  if mname ~= resolvedSequenceName then
    return false
  end
